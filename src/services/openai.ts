...
import type { textCompletionModel, chatCompletionModel } from "../types/openAIModelSettings";
import {
    ChatPromptTemplate,
...
const createChatCompletionModel = async (systemMessage: string, settings?: chatCompletionModel) => {
    const model = new ChatOpenAI({
        temperature: settings?.temperature || 0.9, streaming: settings?.streaming || true,
        callbacks: [
            {
                handleLLMNewToken(token: string) {
                    process.stdout.write(token);
